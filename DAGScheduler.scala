...
private[spark] class DAGScheduler(
  ...
  //==================
  private val OptimalDynamicScheduling = sc.getConf.getBoolean("spark.yarn.optimalDynamicSchedule",false)
  private val OptimalDynamicScheduleShuffle = sc.getConf.getBoolean("spark.yarn.optimalDynamicScheduleShuffle",false)
  private val BlackHostList = List("medusa018","medusa024","medusa025","medusa030","medusa034","medusa035")  // 文件分布实在没办法才会执行在这些节点上
  //==================
  /**
  ...
  private def submitMissingTasks(stage: Stage, jobId: Int) {
    ...
    var taskIdToLocations: Map[Int, Seq[TaskLocation]] = try {
      stage match {
        // id:partitionId
        case s: ShuffleMapStage =>
          partitionsToCompute.map { id => (id, getPreferredLocs(stage.rdd, id)) }.toMap  // 确实是在这里获取的数据信息
        case s: ResultStage =>
          partitionsToCompute.map { id =>
            val p = s.partitions(id)
            (id, getPreferredLocs(stage.rdd, p)) // 确实是在这里获取的数据信息
          }.toMap
      }
    } catch {
      case NonFatal(e) =>
    ...
    logInfo("-------------------start---------------------")
    logInfo("0_taskIdToLocations: "+taskIdToLocations.toString())  // 为什么会没有内容?
    // 固有策略已经不需要跑这里了
    if(OptimalDynamicScheduling){

      if(!OptimalDynamicScheduleShuffle){
        // taskIdToLocations = taskIdToLocations.mapValues(_.take(1)) // 1种策略
        taskIdToLocations = OnePreferLacation(taskIdToLocations)
      }else {
        // 出问题
        logInfo("1_taskIdToLocations: "+taskIdToLocations.toString())
        // 或许真的是因为这句话出现的问题mapValues(f => Random.shuffle(f)) 而应该是：
        // case (key, value) => key -> Random.shuffle(value)
        // taskIdToLocations = taskIdToLocations.mapValues(f => Random.shuffle(f))// 另一种策略！但是选择的方式还是远远不及你方法算法的优势

        // 这种方式 能够很有效地避免负载集中造成的运行时间提高 core并非意味着都是同计算效率的 具体还是以集群节点的负载为主
        taskIdToLocations = taskIdToLocations.map { case (key, value) =>
          key -> Random.shuffle(value) // 多了层这个 负载均衡性
        }
        taskIdToLocations = OnePreferLacation(taskIdToLocations)
        logInfo("2_taskIdToLocations: "+taskIdToLocations.toString())
        // taskIdToLocations = taskIdToLocations.mapValues(_.take(1)) // 注意:不同的运行可能返回不同的结果，除非底层集合类型是有序的
      }
      // 现在 var taskIdToLocations: Map[Int, Seq[TaskLocation]]
      // Map(0 -> List(medusa027), 5 -> List(medusa005), 1 -> List(medusa029), 6 -> List(medusa035),
      // 2 -> List(medusa036), 7 -> List(medusa026), 3 -> List(medusa005), 4 -> List(medusa026))
      logInfo("3_taskIdToLocations: "+taskIdToLocations.toString())
      logInfo("-------------------end---------------------")
    }
    ...
    val tasks: Seq[Task[_]] = try {
      ...
      stage match {
        case stage: ShuffleMapStage =>
          ...
          partitionsToCompute.map { id =>
            val locs = taskIdToLocations(id) // val locs: Seq[TaskLocation]
            //logInfo("[RS] taskId:"+id+"====>locs:"+locs.toString())
            val part = partitions(id)
            //logInfo("[RS] partitionIndex:"+part.index)
        ...
        case stage: ResultStage =>
          partitionsToCompute.map { id =>
            ...
            //logInfo("[RS] partitionIndex:"+part.index)
            // 究极奇怪的bug
            //logInfo("4_taskIdToLocations:"+taskIdToLocations)
    ...
    if (tasks.size > 0) {
      ...
      taskScheduler.submitTasks(new TaskSet(
        tasks.toArray, stage.id, stage.latestInfo.attemptNumber, jobId, properties))  // DAGScheduler构建Tasks阶段结束，将task构建成TaskSet，然后交给TaskScheduler
    } else {
  ...
  /* 新增方法: 修改taskIdToLocation的算法*/
  private def OnePreferLacation(taskIdToLocations:Map[Int, Seq[TaskLocation]]):Map[Int, Seq[TaskLocation]] = {
    val intToLocations: Seq[TaskLocation] = taskIdToLocations(0) // 有些时候是没有数据偏好位置的
    if(intToLocations.nonEmpty){
    taskIdToLocations.map { case (id, hosts) =>
      var chooseHost = hosts.head // 取第一个元素作为初始值
      var value = hosts // 中间辅助变量
      breakable {
        while (BlackHostList.contains(chooseHost.toString)) {
          if (value.size == 1) {
            chooseHost = value.head
            break();
          }
          value = value.tail // 除去头部一个
          chooseHost = value.head
        }
      }
      id -> Seq(chooseHost)}
