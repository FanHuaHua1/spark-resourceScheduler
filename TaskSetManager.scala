...
private[spark] class TaskSetManager(
  ...
  //==============================
  // 没法局部改变网速  全局改变网速就直接自己手动请求调速就好
//  private val networkIOHost = conf.getOption("spark.yarn.executor.io.sshHost")
//  private val networkValue = conf.get("spark.yarn.executor.io.value","30")
//  private var netChangeFlag: Boolean = false
//  private val networkIOTime = conf.getInt("spark.yarn.executor.io.time", 120) // 用于获取网速csv
  ...
  for (i <- (0 until numTasks).reverse) {
    addPendingTask(i)  // 内部调用代码块 构建TaskSetManager时会通过addPendingTasks函数将所有的task 调用addPendingTask函数，将待处理Task的按照Task的偏好位置，添加到不同的缓存队列中。
  }
  ...
  // 构建好了各种队列后，TaskSetManager还调用了一个computeValidLocalityLevels函数，
  // 对外开放 taskSetManager中的 tasks 都有哪些locality levels，以便在后续TaskSechduler调度和延迟调度 tasks 时发挥作用。
  // myLocalityLevels: Array[TaskLocality.TaskLocality] 是当前 TaskSetManager 的所有 tasks 所包含的本地性（locality）集合，本地性越高的 locality level 在 myLocalityLevels 中的下标越小
  ...
  private[spark] def addPendingTask(index: Int) {
    ...
    for (loc <- tasks(index).preferredLocations) { // 这个偏好竟然跟taskIdToLocations不对应？
      // loc:单个hostName
      //logInfo("[RS] tasks("+index+").preferredLocations: "+loc+" size:"+tasks(index).preferredLocations.size)
      loc match {
        ...
        case _ => // logInfo("[RS] exe match _") 一直都执行这里
      }
      // 这俩要注意，无论是不是executor级别，都将放入到host容器中，如果存在rack，则也要放入到rack容器中。
    ...
    // logInfo("BBBBBBBBBBBBBBBBBBBBBBB")
    // logInfo("[RS] pendingTasksForHost: "+pendingTasksForHost.toString())
    // logInfo("[RS] pendingTasksForRack: "+pendingTasksForRack.toString())
    // logInfo("BBBBBBBBBBBBBBBBBBBBBBB")
   ...
   *         而在dequeueTask函数中，则是根据本地策略去forExecutor，pendingTasksForHost等队列中获取对应的分区下标等参数构建成一个tuple类型
   */
    ...
    : Option[(Int, TaskLocality.Value, Boolean)] =
  {
    ...
    if (TaskLocality.isAllowed(maxLocality, TaskLocality.NODE_LOCAL)) {
      for (index <- dequeueTaskFromList(execId, host, getPendingTasksForHost(host))) { //  dequeueTaskFromList这里选的？
        return Some((index, TaskLocality.NODE_LOCAL, false))  // tuple类型
      }
    }
   ...
   * 终于回到了TaskSetManager.scala函数中
   */
  @throws[TaskNotSerializableException]
  def resourceOffer(  // 一个一个已经分配好的executor随机（execId shuffle）传进来  接下来就是看任务分配到哪了
      execId: String,
      host: String,
    ...
    : Option[TaskDescription] =
  {
   ...
   /*     // val bool: Boolean = taskLocality.toString.equals("NODE_LOCAL") || taskLocality.toString.equals("PROCESS_LOCAL")
        // if(!bool && networkIOHost.isDefined && !runExecutorsHostsSet.contains(host)){
          if(networkIOHost.isDefined && !netChangeFlag){
            // 既然是全局调控 还是不要在代码中去发送请求了 自己手动调控集群吧(*^_^*)
            // 在阶段1就会发送了 看这个TaskSetManager到底有多少对象了。。。 如果每个阶段都有一个这个对象那岂不是每个阶段都会发送一次
            netChangeFlag = true // 只执行一次
            // runExecutorsHostsSet.add(host)
            // 发送请求  统计网络IO信息
            // val url = new URL(s"http://${networkIOHost.get}:9898/io?hostName=${host}&time=${networkIOTime}")
            val url = new URL(s"http://${networkIOHost.get}:9898/changeNet?value=${networkValue}")
            // logInfo("[RS] wangSuXianZhi:"+host)
            val connection = url.openConnection().asInstanceOf[HttpURLConnection]
            connection.setRequestMethod("GET")
            val responseCode = connection.getResponseCode
            // logInfo("[RS] "+host+":"+taskLocality.toString)
            logInfo("[RS] getIoInfoResponseCode: " + responseCode)
        ...
        // 返回tuple类型后回到TaskSetManager的resourceOffer函数，这里会根据返回的tuple类型构建成一个TaskDescription类返回给上层调用resourceOffer函数的TaskSchedulerImpl
        new TaskDescription(
          taskId,
          attemptNum,
          execId,
  ...
  private def computeValidLocalityLevels(): Array[TaskLocality.TaskLocality] = {
    ...
    val levels = new ArrayBuffer[TaskLocality.TaskLocality]  // 莫非关键是在这里
    ...
    if (!pendingTasksForHost.isEmpty &&
      ...
      // 给定的代码将遍历 pendingTasksForHost 中的键集合，
      // 并对每个键调用 hasExecutorsAliveOnHost 方法。
      // 如果存在至少一个键满足 hasExecutorsAliveOnHost 返回 true 的条件，则整个表达式的结果为 true，否则为 false。
      levels += NODE_LOCAL
    }
